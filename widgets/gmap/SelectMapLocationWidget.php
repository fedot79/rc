...
class SelectMapLocationWidget extends InputWidget
{
    ...
    public $attributeStreet;
    public $attributeHouse;
...
//        }
        ...
        $region = Html::getInputId($this->model, $this->attributeRegion);
        $street = Html::getInputId($this->model, $this->attributeStreet);
        $house = Html::getInputId($this->model, $this->attributeHouse);
        ...
        if (strpos($this->field->template, '{map}') === false) {
            $this->field->template = preg_replace('/\{input\}/', '{map}{input}', $this->field->template);
        }
